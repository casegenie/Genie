...
public class Broadcast extends RPC {
    ...
    public RPC init(FriendConnection rpcc, int fromGuid, int numHops) {
        ...
        if (payload != null) manager.getBroadcastManager().checkHash(srcGuid, msgId, payload.length);
        return this;
    }
    ...
    public void execute(Packet in) throws IOException {
        ...
        if (!manager.getBroadcastManager().checkHash(srcGuid, msgId, payloadLen)) {
            if(T.t)T.debug("Dropping already broadcasted message "+this);
            in.skip(payloadLen);
            return;
        }

        if(T.t)T.debug("Executing broadcasted message. It came from "+hops +" hops away.");
        in.mark();
        ...
        if (!r.isBroadcastable()) {
            if(T.t)T.warn("Someone trying to Broadcast non-broadcastable package! "+r.getClass().getName());
            return;
        }
        in.reset();
        ...
        for(Friend f : manager.friends()) {
            if (f.isConnected() && f.getFriendConnection() != null && f.getFriendConnection() != con) {
               if(T.t)T.info("Sending Broadcast to "+f+" remote: "+con.getRemoteFriend()+" con: "+con+" rc: "+f.getFriendConnection());
