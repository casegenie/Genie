...
public class Router {
    ...
    public void updateRouteTable(Friend friend, int nodeGuid, int hops) {
        if (manager.getFriend(nodeGuid) != null) return;
        if (hops == 1) {
            if(T.netTrace)T.trace("Number of hops is 1. Can find this user using the friend managers friends of friends. Don't add him to the route table - this saves a lot of memory");
            return;
        }
    ...
    public Friend findClosestFriend(int nodeGuid) {
        ...
        if (manager.getUntrustedNode(nodeGuid) != null) {
            ...
            for(Friend friend : manager.friends()) {
                if (friend.getFriendsFriends().get(nodeGuid) != null) return friend;
            }
        }
