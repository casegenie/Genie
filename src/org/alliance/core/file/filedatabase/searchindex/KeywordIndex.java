...
public class KeywordIndex {
    ...
    public KeywordIndex() {
        for (FileType ft : FileType.values()) {
            fileTypeFilters.put(ft, new BitSet());
        }
    }

    public synchronized void add(int index, FileDescriptor fd) {
        for (FileType ft : FileType.values()) {
            if (ft.fileTypeIdentifier().matches(fd)) {
                fileTypeFilters.get(ft).set(index);
            }
        }
            ...
            while (st.hasMoreTokens()) {
                ...
                for (String keyword : index.keySet()) { //find all indexed keywords that contain the string s
                    if (keyword.indexOf(s) != -1) {
                        ...
                        if (b != null) {
                            totalForKeyword.or(b.decompress());
                        }
                    }
                }
                if (totalForKeyword.cardinality() == 0) {
                    return new int[0]; //no hits for one of the keywords - no need to go on.
                }
    ...
    public synchronized void load(ObjectInputStream in) throws IOException, ClassNotFoundException {
        if (in.readByte() != VERSION) {
            throw new IOException("Incorrect version for keyword index!");
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (T.t) {
                T.info("Loaded bitset for " + ft + ", hits: " + bs.cardinality());
            }
        ...
        for (FileType ft : FileType.values()) {
            if (!fileTypeFilters.containsKey(ft)) {
                fileTypeFilters.put(ft, new BitSet());
            }
        }
    }
    ...
    public void remove(int index) {
        if (T.t) {
            T.info("Removing index " + index + " from keywordindex.");
        }
        for (CompressedBitSet b : this.index.values()) {
            b.clear(index);
        }
    }
