...
public class FileDescriptorAllocationTable {
    ...
    public int getOffset(int fdIndex) {
        if (fdIndex >= numberOfFiles) {
            throw new ArrayIndexOutOfBoundsException("No such fd index " + fdIndex);
        }
    ...
    public int addOffset(int offset) {
        ...
        if (numberOfFiles > offsets.length) {
            if (T.t) {
                T.info("Expanding FDAT with " + ARRAY_INCREMENT_SIZE + " new size: " + (offsets.length + ARRAY_INCREMENT_SIZE) + " nof : " + numberOfFiles);
            }
    ...
    public void save(ObjectOutputStream out) throws IOException {
        ...
        for (int i = 0; i < numberOfFiles; i++) {
            out.writeInt(getOffset(i));
        }
    }

    public void load(ObjectInputStream in) throws IOException {
        if (in.readInt() != MAGIC) {
            throw new IOException("Index file is probably corrupt! Magic number incorrect.");
        }
